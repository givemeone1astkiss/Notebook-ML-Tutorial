...
$$
\mathcal{J}=\sum_{n=1}^N\sum_{k=1}^Kr_{nk}\|x_n-\mu_k\|^2
$$
...
根据潜在变量的边缘概率分布 $p(z)$ 和条件概率分布 $p(x|z)$ 定义联合概率分布 $p(x,z)$ :
$$
p(x|z)=\prod_{k=1}^K\mathcal{N}(x|\mu_k,\Sigma_k)^{z_k}
$$

$$
p(x)=\sum_zp(x.z)=\sum_zp(z)p(x|z)=\sum_{k=1}^K\pi_k\mathcal{N}(x|\mu_k,\Sigma_k)
$$

另一个重要的量是隐变量的后验分布 $p(z|x)$，可以由贝叶斯定理推出：
$$
\gamma(z_k)=p(z=1|x)=\frac{p(z_k=1)p(x|z_k=1)}{\sum_{i=1}^Kp(z_i=1)p(x|z_i=1)}=\frac{\pi_k\mathcal{N}(x|\mu_k,\Sigma_k)}{\sum_{i=1}^K\pi_i\mathcal{N}(x|\mu_i,\Sigma_k)}
$$
...
![GMM](D:\Desktop\Notebook-ML-Tutorial\images\10.1.png)
...
$$
\mathcal{N}(x_n|x_n,\sigma_j^2I)=\frac{1}{(2\pi)^\frac{1}{2}\sigma_j^2}
$$
...
![GMM](D:\Desktop\Notebook-ML-Tutorial\images\10.2.png)
...
首先考虑似然函数优化的约束条件，令 $\ln p(X|\pi,\mu,\Sigma)$ 关于高斯分量的均值 $\mu_{k}$ 的导数为零：
$$
\sum_{n=1}^N\frac{\pi_k\mathcal{N}(x_n|\mu_k,\Sigma_k)}{\sum_j\pi_j\mathcal{N}(x_n|\mu_j,\Sigma_j)}\Sigma_k^{-1}(x_n-\mu)=\sum_{n=1}^Nr(z_{nk})\Sigma_k^{-1}(x_n-\mu)=0
$$
然后整理可以得到：
$$
...
$$
L(\pi_k)=\ln(X|\pi,\mu,\Sigma)+\lambda\left(\sum_{k=1}^K\pi_k-1\right)\\
\frac{\partial L}{\partial \pi_k}=\sum_{n=1}^N\frac{\mathcal{N}(x_n|\mu_k,\Sigma_k)}{\sum_j\pi_j\mathcal{N}(x_n|\mu_j,\Sigma_j)}+\lambda
$$
化简后可得：
$$
\pi_k=\frac{N_k}{N}
...
$$
\gamma(z_{nk})=\frac{\mathcal{N}(x_n|\mu_k,\Sigma_k)}{\sum_j\pi_j\mathcal{N}(x_n|\mu_j,\Sigma_j)}
$$
...
$$
一个重要的观察是对潜在变量的求和位于对数内部，这使得即使联合概率分布 $p(X,Z|\theta)$ 属于指数族分布，由于这个求和式的存在，边缘概率分布 $p(X|\theta)$ 往往也不是指数族分布。求和的存在使得似然函数变得复杂。

假定对于每个观测，都有潜在变量的对应值，我们将 $\{X,Z\}$称为完整数据集，完整数据集的对数似然函数形式为 $\ln p(X,Z|\theta)$，我们假定对这个对数似然函数进行最大化是简单的。

然而实际上，我们并不能取得完整数据集，我们关于潜在变量 $Z$ 的知识仅仅来源于后验概率分布 $p(Z|X,\theta)$。由于我们不能使用完整数据的对数似然函数，因此我们反过来考虑它在后验概率分布下的期望，这对应 E 步骤，在接下来的 M 步骤中，我们对这个期望进行最大化。

在 E 步骤中，我们使用旧参数计算潜在变量的后验概率分布 $p(Z|X,\theta^{old})$，然后根据这个后验概率计算完整数据集似然函数的期望，这是一个关于一般参数的函数：
$$
\mathcal{Q}(\theta,\theta^{old})=\sum_Zp(Z|X,\theta^{old})\ln p(X,Z|\theta)
$$
...
$$
p(X,Z|\mu,\Sigma,\pi)=\prod_{n=1}^N\prod_{k=1}^K[\pi_k\mathcal{N}(x_n|\mu_k,\Sigma^k)]^{z_{nk}}
$$
对数形式为：
$$
p(X,Z|\mu,\Sigma,\pi)=\sum_{n=1}^N\sum_{k=1}^Kz_{nk}[\ln\pi_k+\ln\mathcal{N}(x_n|\mu_k,\Sigma^k)]
$$
在这个形式下，对数运算直接作用于高斯分布，因此这样的形式是易于优化的。

然后我们考虑完整数据对数似然函数关于潜在变量后验概率分布的期望，这个后验概率分布的形式是：
$$
p(Z|X,\mu,\theta,\pi)\propto\prod_{n=1}^N\prod_{k=1}^K[\pi_k\mathcal{N}(x_n|\mu_k,\Sigma^k)]^{z_{nk}}
$$
在这个后验分布下，指示值的期望为：
$$
\mathbb{E}[z_{nk}]=\frac{\sum_{z_n}z_{nk}\prod_{k'}[\pi_{k'}\mathcal{N}(x_n|\mu_{k'},\Sigma_{k'})]^{z_{nk}}}{\sum_{z_n}\prod_j[\pi_j\mathcal{N}(x_n|\mu_j,\Sigma_j)]^{z_{nk}}}=\frac{\pi_k\mathcal{N}(x|\mu_k,\Sigma_k)}{\sum_{i=1}^K\pi_i\mathcal{N}(x|\mu_i,\Sigma_k)}=\gamma(z_{nk})
$$
于是完整数据的对数似然函数的期望是：
$$
\mathbb{E}_Z[\ln p(X,Z|\mu,\Sigma,\pi)]=\sum_{n=1}^N\sum_{k=1}^K\gamma(z_{nk})[\ln\pi_k+\ln\mathcal{N}(x_n|\mu_k,\Sigma_k)]
$$
...
$$
\gamma(z_{nk})=\frac{\pi_k\exp\left\{-\frac{\|x_n-\mu_k\|^2}{2\epsilon}\right\}}{\sum_j\pi_j\exp\left\{-\frac{\|x_n-\mu_k\|^2}{2\epsilon}\right\}}
$$
对于 $\epsilon\rightarrow 0$ 的极限情况，分类由软分类退化为硬分类，每个数据点都被分配为距离最近的均值的聚类。那么对均值的 EM 估计就简化为了 K-means 的形式。在这种情况下，完整数据的对数似然就变成了 ：
$$
\mathbb{E}_Z[\ln p(X,Z|\mu,\Sigma,\pi)]\rightarrow-\frac{1}{2}\sum_{n=1}^N\sum_{k=1}^Kr_{nk}\|x_n-\mu_k\|^2+\mathrm{const}
$$
...
$$
直接对 $p(X|\theta)$ 进行优化是困难的，而优化 $p(X,Z|\theta)$ 则容易得多。接下来，引入一个定义在潜在变量空间上的分布 $q(z)$，对于任意的 $q(z)$，有以下的分解成立：
$$
\ln p(X|\theta)=\mathcal{L}(q,\theta)+KL(q\|p)\\
\mathcal{L}(q,\theta)=\sum_Zq(Z)\ln\left[\frac{p(X,Z|\theta)}{q(Z)}\right]\\
KL(q(Z)\|p(Z|X))=-\sum_Zq(Z)\ln\left[\frac{p(Z|X,\theta)}{q(Z)}\right]
$$
$\mathcal{L}(q,\theta)$ 是一个关于 $\theta$ 的函数，也是一个关于 $q(Z)$ 的泛函，$\mathcal{L}(q,\theta)$ 包含了联合概率分布，而 $KL(q\|p)$ 包含了条件概率分布。由于 $KL(q\|p)=-\sum_Zq(Z)\ln\left[\frac{p(Z|X,\theta)}{q(Z)}\right]\geq0$ ，当且仅当 $q(Z)=p(Z|X)$ 时取等，因此可以认为 $\mathcal{L}(q,\theta)$ 是对数似然 $ln p(X|\theta)$ 的下界。

在 E 步中，下界 $\mathcal{L}(q,\theta)$ 关于 $q(Z)$ 被最大化，此时有 $\ln p(X|\theta)=\mathcal{L}(q,\theta)$，而 $\theta$ 保持不变，在接下来的 M 步中， $q(Z)$ 保持不变，下界关于 $\theta$ 进行最大化，这会使得对应的对数似然函数增大，由于概率分布 $q(Z)$ 由旧的参数值决定，并且在 M 步中保持固定，因此它不会与新的后验概率分布一致，同时 $KL(q\|p)\not ={0}$，因此，对数似然函数的增加量大于下界的增加量。

如果将 $q(Z)=p(Z|X,\theta^{old})$ 代入 $\mathcal{L}(q,\theta)$，在 E 步之后，$\mathcal{L}(q,\theta)$ 的形式是：
$$
\begin{aligned}
\mathcal{L}(q,\theta)&=\sum_Zp(Z|X,\theta^{old})\ln p(X,Z|\theta)-\sum_Zp(Z|X,\theta^{old})\ln p(Z|X,\theta^{old})\\
&=\mathcal{Q}(\theta,\theta^{old})+\mathrm{const}
\end{aligned}
$$
因此，对 ELBO 的优化实际上等价于对完整数据对数似然期望的优化。

![GMM](D:\Desktop\Notebook-ML-Tutorial\images\10.3.png)
